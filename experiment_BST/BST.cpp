...
/*中序遍历二叉树, root为指向二叉树根结点的指针*/
{
	if(root != NULL)
	{
		InOrderCleanFlag(root->lchild);
		root->flag = 0;
		printf("%d ",root->key);
		InOrderCleanFlag(root->rchild);
	}
}
...
int main(int argc, char *argv[])
{
	...
	SearchBST(T,562);
	MakeDot(T);
	system("dot.exe -Tpng bst.gv -o bst_search(562).png");
	
	PreOrderCleanFlag(T);
	SearchBST(T,193); 
	MakeDot(T);
	system("dot.exe -Tpng bst.gv -o bst_search(193).png");
	
	PreOrderCleanFlag(T);
	SearchBST(T,832); 
	MakeDot(T);
	system("dot.exe -Tpng bst.gv -o bst_search(832).png");
